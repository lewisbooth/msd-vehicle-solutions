...
- Before enabling contact email, add a public form rate limit or Turnstile and align the form's `telephone` honeypot with the API's `website` field; verify real delivery and the phone fallback. Current email binding remains absent, so submissions are rejected.
