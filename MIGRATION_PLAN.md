...
- [ ] Serve production R2 photos through an R2 custom domain with Cloudflare edge caching; confirm a warm `CF-Cache-Status: HIT` and preserve immutable browser caching. The preview `r2.dev` hostname is for functional tests and does not exercise the edge cache.
...
- [ ] Make preview API latency representative of UK visitors: the preview D1 primary is currently `ENAM` with replication disabled, whereas the already provisioned production D1 primary is `WEUR`. Recreate the isolated preview DB with a `weur` location hint and reimport only current live records before performance benchmarking, or benchmark the production `WEUR` binding safely during cutover rehearsal.
...
- Workers Builds Preview defaults and existing branch have the correct build/deploy commands. The no-pending D1 check and preview deployment passed in CI; the build token's D1 **edit** permission and rollback behavior cannot be confirmed without an actual needed migration or a separate isolated rehearsal. The initial schema migration was applied manually to preview D1. No useless schema change should be manufactured solely for a CI test.
...
- Preview's R2 `r2.dev` URL skips Cloudflare edge caching; its one-year `Cache-Control` only helps repeat browser requests. Production photos need an R2 custom domain and a measured cache hit. Preview's `ENAM` D1 is not representative of the production `WEUR` D1 for UK visitors.
