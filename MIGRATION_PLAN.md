...
- Workers Builds currently runs `npx wrangler preview` on the existing branch; its separate branch override masks the Preview defaults. Fix both settings as logged above, and ensure the build token has D1 edit access so `npm run migrate:preview` can run before deployment. The connected API cannot edit Builds (`10000`); the initial migration was applied manually to isolated preview D1. There is no useful additive schema change to manufacture solely for a CI test: first verify a branch build runs the migration command, then use an actual needed migration to prove D1 write permissions when one exists.
