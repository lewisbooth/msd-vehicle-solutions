...
- **2026-09-25:** Investigated Firefox `NS_BINDING_ABORTED` rows for `/customs`. All four referenced image URLs return full `200` responses; Firefox had cancelled requests, with no evidence of missing Cloudflare files. The current unversioned brochure images revalidated on every visit. Extended build-time content fingerprinting to 53 bundled images/favicons and switched CSS font URLs to Vite-managed hashed files, retaining short-lived/revalidated document URLs. Local `npm run build` passes; branch deployment and live header checks are next.
